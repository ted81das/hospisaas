    ...
    function getPatient()
    {
        ...
        foreach ($data['patients'] as $patient) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist', 'Laboratorist', 'Nurse', 'Doctor'))) {
                $options1 = '<a type="button" class="btn btn-primary editbutton btn-sm mb-1" title="' . lang('edit') . '" data-toggle="modal" data-id="' . $patient->id . '"><i class="fa fa-edit"></i> ' . lang('') . '</a>';
            }

            $options2 = '<a class="btn btn-info detailsbutton btn-sm mb-1" title="' . lang('info') . '" href="patient/patientDetails?id=' . $patient->id . '"><i class="fa fa-info"></i> ' . lang('') . '</a>';

            $options3 = '<a class="btn btn-info btn-sm mb-1" title="' . lang('details') . '" href="patient/medicalHistory?id=' . $patient->id . '"><i class="fa fa-stethoscope"></i> ' . lang('details') . '</a>';

            $options4 = '<a class="btn btn-secondary btn-sm mb-1" title="' . lang('payment') . '" href="finance/patientPaymentHistory?patient=' . $patient->id . '"><i class="fa fa-money-bill-alt"></i> ' . lang('payment') . '</a>';

            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist', 'Laboratorist', 'Nurse', 'Doctor'))) {
                $options5 = '<a class="btn btn-danger delete_button btn-sm mb-1" title="' . lang('delete') . '" href="patient/delete?id=' . $patient->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"></i> ' . lang('') . '</a>';
            }

            $options6 = '<a type="button" class="btn btn-warning detailsbutton inffo btn-sm mb-1" title="' . lang('info') . '" data-toggle="modal" data-id="' . $patient->id . '"><i class="fa fa-info"></i> ' . lang('') . '</a>';

            if ($this->ion_auth->in_group('Doctor')) {
                $options7 = '<a class="btn btn-success detailsbutton btn-sm mb-1" title="' . lang('instant_meeting') . '" href="meeting/instantLive?id=' . $patient->id . '" onclick="return confirm(\'Are you sure you want to start a live meeting with this patient? SMS and Email will be sent to the Patient.\');"><i class="fa fa-headphones"></i> ' . lang('start_live') . '</a>';
            } else {
                $options7 = '';
            }

            $cases = '<a class="btn btn-success btn-sm mb-1" title="' . lang('cases') . '" href="patient/patientCaseList?id=' . $patient->id . '"><i class="fa fa-eye"></i> ' . lang('cases') . '</a>';
            ...
            <i class="fas fa-bars"></i> ' . lang('actions') . ' <span class="caret"></span>
                ...
                <ul class="dropdown-menu">
                    ' . ($options1 ? '<li><a class="editbutton" title="' . lang('edit') . '" data-toggle = "modal" data-id="' . $patient->id . '">  <i class="fa fa-edit"></i> ' . lang('edit') . '</a></li>' : '') . '
                    ' . ($options6 ? '<li><a class="detailsbutton inffo"' . lang('info') . '" data-toggle = "modal" data-id="' . $patient->id . '"> <i class="fa fa-file-invoice"></i> ' . lang('info') . ' </a></li>' : '') . '
                    ' . ($options3 ? '<li><a href="' . site_url("patient/medicalHistory?id=" . $patient->id) . '"> <i class="fa fa-print"></i> ' . lang('medical_history') . ' </a></li>' : '') . '
                    ' . ($options4 ? '<li><a href="' . site_url("finance/patientPaymentHistory?patient=" . $patient->id) . '" > <i class="fa fa-money-check"></i> ' . lang('payment') . ' ' . lang('history') . ' </a></li>' : '') . '
                    ' . ($options5 ? '<li><a href="' . site_url("patient/delete?id=" . $patient->id) . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"> <i class="fa fa-trash"></i> ' . lang('') . ' </a></li>' : '') . '
